...
def order_points(points1):
    # make sure the end of the chain is the last point
    x0 = points1[0][0]
    x1 = points1[-1][0]

    # check which point is closer to the edge of the image
    # if the point is closer to the edge of the image, then it is the end of the chain
    if x0 < x1:
        return points1[::-1]
    else:
        return points1
...
def get_points(
    ...
    # ensure that the points are are ordered from
    # the guidewire tip to the base because ... annotators 🙁
    points1 = order_points(points1)
    points2 = order_points(points2)
